...
func (p *ServerInstanceInfo) Reload() error {
	...
	if len(instanceInfos) != 0 && p.firstGetInstanceTime == 0 {
		p.firstGetInstanceTime = time.Now().Unix()
		log.Infof("get instance info: %+v", instanceInfos)
	}

	// 60 seconds after the data is obtained, the request frequency is reduced to once per minute
	if p.firstGetInstanceTime > 0 && time.Now().Unix()-p.firstGetInstanceTime > 60 {
