...
impl L7ProtocolParserInterface for FastCgiLog {
    ...
    fn parse_payload(&mut self, payload: &[u8], param: &ParseParam) -> Result<L7ParseResult> {
        ...
        let is_php = || {
            /*
                due to php read the first 8 bytes first, when proc is php, need to skip the header and parse the param directly
                FIXME: remove when implement tcp reassemble
            */
        ...
        match param.direction {
            PacketDirection::ClientToServer => {
                ...
                if is_php() {
                    info.fill_from_param(payload, param.direction, config)?;
                } else {
                    for (record, record_payload, off) in RecordIter::new(payload) {
                        if record.record_type == FCGI_PARAMS {
                            ...
                            if record.content_len > 0 {
                                ...
                                info.seq_off = (off + FCGI_RECORD_FIX_LEN) as u32;
                                break;
                            }
                        }
                    }
            ...
            PacketDirection::ServerToClient => {
                ...
                if is_php() {
                    ...
                    'l: for (record, record_payload, _) in RecordIter::new(payload) {
